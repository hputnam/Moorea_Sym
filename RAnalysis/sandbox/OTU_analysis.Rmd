...
load("../Data/Moorea_97glo_sym.RDada"); phy97glo.f <- phy.f # loads phyloseq object of 97% global OTUs, renames to phy97glo.f
load("../Data/Moorea_sym.RData"); phy97.f <- phy.f  # loads phyloseq object of 97% within-sample OTUs, renames to phy97.f
...
# Identify OTUs below threshold count
taxa <- taxa_sums(phy97glo.f)[which(taxa_sums(phy97glo.f) >= n)]
# Remove taxa below threshold count
phy97glo.f <- prune_taxa(names(taxa), phy97glo.f)
...
# Remove samples with fewer reads than threshold
phy97glo.f <- prune_samples(sample_sums(phy97glo.f)>=sn, phy97glo.f)
...
tax_table(phy97glo.f) <- as.matrix(get.st(data.frame(tax_table(phy97glo.f), stringsAsFactors=FALSE)))
...
# Convert to proportion (relative abundance)
phy97glo.f.p <- transform_sample_counts(phy97glo.f, function(x) x/sum(x))
# Apply transformation function
transform <- function(x) sqrt(x/sum(x))  # Set transformation function
phy97glo.f.t <- transform_sample_counts(phy97glo.f, transform)  # Transform data
```

#### Descriptive stats of Filtered datasets
```{r phy.f_histograms, fig.height=4, fig.width=8}
# Compute summary statistics
stats97.f <- data.frame(`97% within-sample OTUs`=t(phystats(phy97.f)), check.names=F)
stats97glo.f <- data.frame(`97% global OTUs`=t(phystats(phy97glo.f)), check.names=F)
...
taxhist97glo <- hist(log10(taxa_sums(phy97glo.f)), plot=F)
taxhist100 <- hist(log10(taxa_sums(phy100.f)), plot=F)
samhist97 <- hist(log10(sample_sums(phy97.f)), plot=F)
samhist97glo <- hist(log10(sample_sums(phy97glo.f)), plot=F)
samhist100 <- hist(log10(sample_sums(phy100.f)), plot=F)

par(mfrow=c(2, 3), mar=c(3,3,1,1))
plot(taxhist97, col="black", main="97% within-sample OTU counts", xlim=c(0, 6), las=1, mgp=c(2,0.5,0),
     xlab="No. sequences (log10)", ylab="No. OTUs", cex.lab=0.75, cex.axis=0.75)
plot(taxhist97glo, col="black", main="97% global OTU counts", xlim=c(0, 6), las=1, mgp=c(2,0.5,0),
     xlab="No. sequences (log10)", ylab="No. OTUs", cex.lab=0.75, cex.axis=0.75)
plot(taxhist100, col="black", main="100% OTU counts", xlim=c(0, 6), las=1, mgp=c(2,0.5,0),
     xlab="No. sequences (log10)", ylab="No. OTUs", cex.lab=0.75, cex.axis=0.75)
plot(samhist97, col="black", main="97% within-sample OTU reads per sample", xlim=c(0, 6), las=1, mgp=c(2,0.5,0),
     xlab="No. sequences (log10)", ylab="No. samples", cex.lab=0.75, cex.axis=0.75)
plot(samhist97glo, col="black", main="97% global OTU reads per sample", xlim=c(0, 6), las=1, mgp=c(2,0.5,0),
     xlab="No. sequences (log10)", ylab="No. samples", cex.lab=0.75, cex.axis=0.75)
plot(samhist100, col="black", main="100% OTU reads per sample", xlim=c(0, 6), las=1, mgp=c(2,0.5,0),
     xlab="No. sequences (log10)", ylab="No. samples", cex.lab=0.75, cex.axis=0.75)

# Create stats table
knitr::kable(cbind(stats97.f, stats97glo.f, stats100.f))
```
...
## *Porites sp.*  
```{r por, fig.height=18, fig.width=12}
# Create subsetted phyloseq objects for Porites sp.
por97glo.f.p <- subset_samples(phy97glo.f.p, Species=="Porties")
...
par(mfrow=c(3,1), mar=c(4,4,2,2), mgp=c(0,0.5,0), lwd=0.1)
otubarplot(por97glo.f.p, main="97% global OTUs")
otubarplot(por97.f.p, main="97% within-sample OTUs")
otubarplot(por100.f.p, main="100% OTUs")
```

## *Acropora hyacinthus*  
```{r acr, fig.height=18, fig.width=12}
# Create subsetted phyloseq objects for Acropora hyacinthus
acr97glo.f.p <- subset_samples(phy97glo.f.p, Species=="Acropora")
...
par(mfrow=c(3,1), mar=c(4,4,2,2), mgp=c(0,0.5,0), lwd=0.1)
otubarplot(acr97glo.f.p, main="97% global OTUs")
otubarplot(acr97.f.p, main="97% within-sample OTUs")
