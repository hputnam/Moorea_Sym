...
#include <- c("var", 
#             "daily.mean.range", "daily.range.skewness", "daily.range.kurtosis", "daysover30")

include <- c("var", "daily.mean.range", "daysover30")

#include <- c("mean", "var", "dip", "daily.mean.min", "daily.mean.max", "daily.mean.range", #"daysover30", "daysunder27", "daily.mean.var", "skew")

#include <- c("mean", "var", "daysover30", "daysunder27", "daily.mean.range")
...
sample_data(phy.f)$daily.mean.range <- temp[site.levels, "daily.mean.range"]
...
par(mfrow=c(2,2), mar=c(2.5,2.5,1,1), mgp=c(1.5,0.3,0), tcl=-0.2, xpd=F)
plot.bd.temp("PC1")
plot.bd.temp("PC2")
plot.bd.temp("PC3")
plot.bd.temp("PC4")
...
# Figure 4
```{r Fig4}
tiff("Output/Fig4.tiff", width=4, height=2, units="in", res=300)
...
```{r constrained_ordination site, eval=F}
