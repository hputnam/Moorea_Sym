...
load("Data/tempdata.RData") # loads the temperature data from temperature_analysis
...
# Plot mean daily cycle for each site
time.mean<-aggregate(cbind(temp.all$Site.1,temp.all$Site.2,temp.all$Site.3,temp.all$Site.7,temp.all$Site.8,temp.all$Site.9) ~ temp.all$Time, FUN=mean)
colnames(time.mean) <- c("Time","Site.1","Site.2","Site.3","Site.7","Site.8","Site.9")
time.mean$Time <- as.POSIXct(time.mean$Time, format="%H:%M")
par(mfrow=c(1,1), mar=c(3,3,2,2), mgp=c(1.5,0.1,0), tcl=-0.2)
with(time.mean, {
  plot(Time, Site.1, type="b", ylim=range(time.mean[,2:7]), col=sitecolors[1])
  points(Time, Site.2, type="b", col=sitecolors[2])
  points(Time, Site.3, type="b", col=sitecolors[3])
  points(Time, Site.7, type="b", col=sitecolors[4])
  points(Time, Site.8, type="b", col=sitecolors[5])
  points(Time, Site.9, type="b", col=sitecolors[6])
})
legend("bottom", lty=1, legend=c("Site 1", "Site 2", "Site 3", "Site 7", "Site 8", "Site 9"),
       col=sitecolors, ncol=4, cex=0.7)
...
include <- c("var", "daily.mean.range", "daysover30")
#include <- c("mean", "var", "daily.mean.min", "daily.mean.max", "daily.mean.range",
#             "daysover30", "daysunder27")
...
temp2 <- rbind(apply(as.data.frame(temp.summ), 2, function(x) rev(range(x))),
               as.data.frame(temp.summ))
# Reorder columns
temp2 <- temp2[, c(1,7,13,8,11,2,9)]
# Plot radar chart
rad <- radarchart(
  temp2,
...
# Figure 1
```{r Fig1, eval=T}
tiff("Output/Fig1.tiff", width=10, height=3, units="in", res=300)
par(mfrow=c(1,2), mgp=c(1,0.1,0), tcl=0.25, cex.lab=1, cex.axis=0.8,
    mar=c(2,2,2,2))
layout(matrix(c(1,2,3), nrow=1, ncol=3, byrow = TRUE), heights=c(1,0.8,0.75), widths=c(3.5,3,3.5))

# Panel A
#### MAP OF MOOREA SHOWING SITE LOCATIONS
moorea <- load.image('Output/Moorea_temp_logger_map.png')
plot(moorea, axes=F, rescale=FALSE)
title("a", adj=0)

# Panel B
#biplot(pca.temp, cex=0.9, bty="n", xpd=NA, expand=1.1,
#       ylabs=c("Dip","Daily Mean\nMinimum   ","Mean","Days\nOver 30?C","Variance","Daily Mean  #  \nMaximum   ","Days Under\n27?C"),
#       xlim=c(-0.85,0.75),
#       ylim=c(-0.75,0.75))
#title("b", adj=0)

# Panel B
par(mar=c(1.5,1,1.5,2), xpd=NA)
rad <- radarchart(
  temp2,
  ...
  #custom labels
  vlcex=1, 
  vlabels = c("Mean", "Mean Daily\nMin.", "Days\nunder 27°C", "Mean Daily\nMax.",
              "Days over 30°C", "Variance", "Mean Daily\nRange")
)
title("b", adj=0.05, line = 0)
legend("topright", legend=c("Site 1", "Site 2", "Site 3", "Site 7", "Site 8", "Site 9"), 
       lty=1, lwd=2, col=sitecolors, bty="n", cex=0.9, ncol=2, inset=c(-0.02,-0.05))
dev.off()
```
...
sample_data(phy.f)$mean <- temp.summ[site.levels, "mean"]
sample_data(phy.f)$var <- temp.summ[site.levels, "var"]
#sample_data(phy.f)$dip <- temp[site.levels, "dip"]
sample_data(phy.f)$daily.mean.min <- temp.summ[site.levels, "daily.mean.min"]
sample_data(phy.f)$daily.mean.max <- temp.summ[site.levels, "daily.mean.max"]
sample_data(phy.f)$daily.mean.range <- temp.summ[site.levels, "daily.mean.range"]
sample_data(phy.f)$daysover30 <- temp.summ[site.levels, "daysover30"]
sample_data(phy.f)$daysunder27 <- temp.summ[site.levels, "daysunder27"]
...
# Save OTU table and tax table as supplementary tables
write.csv(otu_table(phy.f.p), file="Output/OTU_table.csv", row.names=F, quote=F)
write.csv(tax_table(phy.f.p), file="Output/tax_table.csv", row.names=F, quote=F)
```
...
tiff("Output/Fig2.tiff", width=3.5, height=5, units="in", res=300)
...
# Figure 3
```{r Fig3}
tiff("Output/Fig3.tiff", width=4, height=4, units="in", res=300)
par(mfrow=c(2,2), mar=c(2,2,1,1), mgp=c(1.1,0.1,0), tcl=0.2, 
    cex.axis=0.8, cex.lab=1)

# Panel A
plot(acr.bd, hull=F, label=F, col=sitecolors, seg.col=sitecolors,
     xlab="PCoA 1", ylab="PCoA 2", main="", cex=1)
ordihull(acr.bd, sample_data(phy.f.t.acropora)$Site,  draw = c("polygon"),
         col = sitecolors, alpha=0.2, lwd=0.05)
...
mtext(side=3, adj=0.95, padj=1.7, substitute(paste(italic("Acropora"))), font=2, cex=0.75)

# Panel B
with(acr.bdsumm, {
  plot(mean, type="n", ylim=c(0, 0.15), ylab="Distance to centroid", xaxt="n", 
       xlab="Site",xlim=c(0.85,6.15))
  points(1:6, mean, cex=1, pch=1:6, col=sitecolors, lwd=2)
  arrows(1:6, mean - se, 1:6, mean + se, length=0.025, angle=90, code=3, lwd=0.5)
  axis(side=1, at=1:6, cex.axis=0.75, labels=c(1,2,3,7,8,9))
...
mtext(side=3, adj=0.95, padj=1.85, substitute(paste(italic("Porites"))), font=2, cex=0.75)
...
temp.dmrange <- as.data.frame(sample_data(phy.f.t)[, "daily.mean.range"])
...
mod.dmrange <- lmer(log(bd$distances) ~ temp.dmrange$daily.mean.range*species + 1|site)

mod.test <- AICc(mod.do30, mod.du27,mod.var, mod.dmrange, mod.mean,  k = 2, REML = T)
...
plot.bd.temp <- function(metric) {
  ...
  plot(NA, xlim=range(acrdat), ylim=c(0,0.125),
  ...
  if (acr.pval < 0.05) {  # If slope is significantly different from 0
    ...
    text(x=par("usr")[2], y=quantile(par("usr")[3:4], 0.9), pos=2, 
  ...
  if (por.pval < 0.05) {  # If slope is significantly different from 0
    ...
    text(x=par("usr")[2], y=quantile(par("usr")[3:4], 0.8), pos=2, 
...
par(mfrow=c(1,2), mar=c(2.5,2.5,1,1), mgp=c(1.5,0.3,0), tcl=-0.2, xpd=F)
plot.bd.temp("PC1")
plot.bd.temp("PC2")
...
plot.bd.temp("daily.mean.range")
# plot.bd.temp("daily.mean.min")
```

# Figure 4
```{r Fig4}
tiff("Output/Fig4.tiff", width=2, height=2, units="in", res=300)
par(mfrow=c(1,1), mar=c(2,2,1,1), mgp=c(1.1,0.1,0), tcl=0.2, 
...
## Constrained ordination -- abbreviated version
Use the three highly correlated temperature metrics that are part of PC1 used in previous analyses, and ask how much of the variability in symbiont community composition can be explained by these three metrics. Do not attempt to disentangle them and just treat them together, as they are treated in PC1.
```{r}
# Porites
ord.porites.CAP2 <- ordinate(phy.f.t.porites,method="CAP",distance="wunifrac",formula= ~ daily.mean.range+daysover30+var)
...
summary(ord.porites.CAP2)$cont$importance[3,3]
vif.cca(ord.porites.CAP2)

# Acropora
ord.acropora.CAP2 <- ordinate(phy.f.t.acropora,method="CAP",distance="wunifrac",formula= ~ var+daily.mean.range+daysover30)
...
summary(ord.acropora.CAP2)$cont$importance[3,3]
vif.cca(ord.acropora.CAP2)
```
...
```{r constrained_ordination temperature metrics, eval=F}

# try each species individually
# PORITES
# First run model selection using PCs
ord.porites.CAP <- ordinate(phy.f.t.porites,method="CAP",distance="wunifrac",formula= ~ PC1)
...
# Since the anova.cca function is broken on the current CRAN release of vegan, create a developer environment where you can use the most updated version 2.5.0 as of 23Jan2018.
# Since we use vegan in multiple places, I didn't want to just load the dev version as the default vegan version.
# I know this is a mess, but it works for now.
dev_mode(on=T)
devtools::install_github("vegandevs/vegan") # Will take a minute to install
unloadNamespace("phyloseq")
unloadNamespace("vegan")
library(vegan)
finalmodel.porites.PCs <- update(finalmodel.porites.PCs) # to work with the dev version of vegan
...
set.seed(2000)
finalmodel.porites.alltempmetrics <- ordistep(ord.porites.CAP2, formula= ~ daily.mean.range+daysover30+var, direction = c("both"), Pin = 0.05, Pout = 0.1, pstep = 100, perm.max = 1000, steps = 50, trace = TRUE)
...
unloadNamespace("vegan")
library(vegan)
finalmodel.porites.alltempmetrics <- update(finalmodel.porites.alltempmetrics) # to work with the dev version of vegan
...
# ACROPORA
# First run model selection using PCs
ord.acropora.CAP <- ordinate(phy.f.t.acropora,method="CAP",distance="wunifrac",formula= ~ PC1)
...
unloadNamespace("vegan")
library(vegan)
finalmodel.acropora.PCs <- update(finalmodel.porites.alltempmetrics) # to work with the dev version of vegan
anova.cca(finalmodel.acropora.PCs, by="axis") 
# These test are significant. This means that the final model (Acropora), distance ~ PC2 + PC4 can be used.unloadNamespace("vegan")
dev_mode(on=F)
...
set.seed(2000)
finalmodel.acropora.alltempmetrics<-ordistep(ord.acropora.CAP2, formula= ~ mean+var+daily.mean.min+daily.mean.max+daysover30+daysunder27, direction = c("both"), Pin = 0.05, Pout = 0.1, pstep = 100, perm.max = 1000, steps = 50, trace = TRUE)
...
anova(finalmodel.acropora.alltempmetrics)
```


```{r}
topten <- names(sort(rowSums(otu_table(phy.f.t.acropora)), decreasing=T)[1:10])
site7 <- rownames(sample_data(phy.f.t.acropora)[which(sample_data(phy.f.t.acropora)$Site==" Site 7"),])
otus <- data.frame(otu_table(phy.f.t.acropora)[topten, site7])
otus$subtype <- as.character(tax_table(phy.f.t.acropora)[rownames(otus), "Subtype"])
otus
View(otus)



topten <- names(sort(rowSums(otu_table(phy.f.t.porites)), decreasing=T)[1:10])
site7 <- rownames(sample_data(phy.f.t.porites)[which(sample_data(phy.f.t.porites)$Site==" Site 7"),])
otus <- data.frame(otu_table(phy.f.t.porites)[topten, site7])
otus$subtype <- as.character(tax_table(phy.f.t.porites)[rownames(otus), "Subtype"])
otus
View(otus)
